...
public class VehicleMovementNormal : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        col = trafficLight.materialZPlus.color;
    ...
    */
    ...
    void FixedUpdate()
    {
        ...
        if (!haventStartedYet && haventEndedYet && transform.position.z > -4)
        {
            ...
            trafficLight.getWaitingTime(myTime, 'Z');
            /*
            trafficLight.totalTimeZ += myTime;
            trafficLight.totalNumberOfCarsZ++;
            Debug.Log(totalNumberOfCars);
            meanZText.text = "Mean Waiting Time in Lane 2 = " + trafficLight.totalTimeZ / totalNumberOfCars;
            */    
        }

        if (!countedOnce && transform.position.z > 5)
        {
            trafficLight.getCountPassed('Z');
            countedOnce = true;
        }
